...
/**
 * @file levi.php
 * @brief File contains https://www.levi.co.za website scraper
 */
 ...
 */
class LeviScraper extends SaleScraper
{
    ...
    {
        parent::__construct('Levi', 'ZAR', $arg);
    ...
    private function parse_catalog($url)
    {
        ...
        $this->category_name = $this->get_node_value('//div[ @class = "category-label sub-cats" ]');
        ...
        // Get pages count
        $pages_count = $this->get_node_value('//ul[ @class = "catalog-pagination pagination-right pull-right" ]/li[ last()-1 ]/a/text()');
        if (!$pages_count)
        {
    ...
    private function parse_item($url)
    {
        ...
        // Product id
        $item['product_id'] = $this->get_node_value('//div[ @class = "catalog-detail__product-sku" ]');
        if (!$item['product_id'])
        {
        ...
        $item['product_id'] = trim(preg_replace('#Item\sNo.#ui', '', $item['product_id']));
        ...
        // Price
        $price = $this->get_node_value('//div[ @class = "catalog-detail__product-price" ]');
        if ($price)
        {
        ...
        // Product option
        $item['product_option'] = $this->get_node_value('//td[ contains(., "Colour  : ") ]/following-sibling::td/text()');
