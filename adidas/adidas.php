...
/**
 * @file dc.php
 * @brief File contains https://shop.adidas.co.za website scraper
 */
 ...
 */
class AdidasScraper extends SaleScraper
{
    ...
    {
        parent::__construct('Adidas', 'ZAR', $arg);
    ...
    private function parse_catalog($url)
    {
        ...
        $this->category_name = $this->get_node_value('//h1[ @class = "current-path--labels" ]');
        ...
        // Get pages count
        $pages_count = $this->get_node_value('//div[ @class = "adidas-pagination__page--number" ]');
        if (!$pages_count)
        {
    ...
    private function parse_catalog_page($html)
    {
        ...
        // Get items
        foreach ( $this->xpath->query('//div[ @class = "catalog--product-wrapper" ]/a') as $node )
        {
    ...
    private function parse_item($url)
    {
        ...
        // Price
        $price = $this->get_node_value('//div[ @itemprop = "price" ]');
        if ($price)
        {
...
$scraper = new AdidasScraper($argv);