...
/**
 * @file dc.php
 * @brief File contains https://www.studio-88.co.za/ website scraper
 */
 ...
 */
class Studio88Scraper extends SaleScraper
{
    ...
    {
        parent::__construct('Studio88', 'ZAR', $arg);
    ...
    private function parse_catalog($url)
    {
        ...
        {
            $items_count = $this->regex('#(\d+)\stotal#', $items_count, 1);
            $this->logg('Items count: '.$items_count);
            $pages_count = ceil($items_count / 12);
    ...
    private function parse_item($url)
    {
        ...
        // Product id
        $item['product_id'] = $this->get_node_value('//span[ contains(., "SKU:") ]/following-sibling::span');
        if (!$item['product_id'])
        {
        ...
        // Image URL
        $item['image_url'] = $this->get_attribute('//img[ @class = "etalage_thumb_image" ]', 'src');
        if (!$item['image_url'])
        {
...
$scraper = new Studio88Scraper($argv);