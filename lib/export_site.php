...
$header = ['Tracking Date', 'Site', 'Brand', 'Product Name', 'Product Description', 'Product ID', 'Product URL', 'Product Image 1', 'Product Image 2', 'Product Image 3', 'Product Image 4', 'Department','Product Category', 'Product Subcategory', 'Product Option', 'Currency Price', 'Currency', 'Price', 'Sale Price'];
//fputcsv($file, $header);
...
foreach ($req as $row)
{
    ...
    $arr[] = filter_text($row['product_department']);
    $arr[] = filter_text($row['category']);
    $arr[] = filter_text($row['product_subcategory']);
...
$query = "SELECT COUNT(DISTINCT product_url) FROM product_tracking WHERE site_id = '$site_id'";
$req = $db->query($query);
if (!$req)
{
    logg('DB error');
...
$delivered = 0;
if ($site_id == 22 || $site_id == 8 || $site_id == 17)	{	//Superbalist or CottonOn or One Day Only
