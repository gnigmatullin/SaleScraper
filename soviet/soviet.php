...
/**
 * @file dc.php
 * @brief File contains https://www.soviet.com/ website scraper
 */
 ...
 */
class SovietScraper extends SaleScraper
{
    ...
    {
        parent::__construct('Soviet', 'ZAR', $arg);
    ...
    private function parse_catalog($url)
    {
        ...
        $this->category_name = $this->get_node_value('//span[ @id = "lblCategoryHeader" ]');
    ...
    private function parse_catalog_page($html)
    {
        ...
        // Get items
        foreach ( $this->xpath->query('//div[ @class = "s-producttext-top-wrapper" ]/h4/a') as $node )
        {
    ...
    private function parse_item($url)
    {
        ...
        // Product id
        $item['product_id'] = $this->get_node_value('//p[ @class = "productCode" ]');
        if (!$item['product_id'])
        {
        ...
        // Product name
        $item['product_name'] = $this->get_node_value('//span[ @id = "lblProductName" ]');
        ...
        // Price
        $price = $this->get_node_value('//span[ @itemprop = "price" ]/span');
        if ($price)
        {
        ...
        // Product option
        $item['product_option'] = $this->get_node_value('//div[ @id = "divColour" ]/div[2]/span');
        
        // Image URL
        $item['image_url'] = $this->get_attribute('//img[ @id = "imgProduct" ]', 'src');
        if (!$item['image_url'])
        {
...
$scraper = new SovietScraper($argv);